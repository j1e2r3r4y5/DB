...
async function handleSubmit() {
    ...
    try {
        // 将配置数据转为十进制字符串存储（与后端 model.AddDevice 的 json tag 对应）
        // 字段映射：后端期望 serial 而非 sn，期望 configdata 而非 config
        let configStr = configInput.value
        if (configType.value === 'hex') {
            const n = parseInt(configInput.value, 16)
            configStr = isNaN(n) ? '30' : String(n)
        }
        // 修改：添加 sendmodel, configdata, baud 字段保存到数据库，与配置下发表单数据同步
        const res = await api.Adddevice({
            Device: [
                {
                    name: form.value.devName,
                    serial: form.value.devSerial,   // 后端 model 用 serial
                    location: form.value.devLocation,
                    sendmodel: form.value.sendmodel, // 发送模式：00=定时,01=线圈置位,02=寄存器变化
                    configdata: configStr,           // 配置数据（定时模式=发送间隔秒数，触发模式=触发地址）
                    baud: form.value.baud            // 波特率
                }
            ]
        })
