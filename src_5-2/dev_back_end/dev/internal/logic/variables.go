...
func (s *sVariable) AddVariable(ctx context.Context, variable *model.Variables) error {
	...
	// 沙箱变量不触发生产设备的 Changeflag
	if variable.Scope != "sandbox" {
...
// BatchAddVariable 批量新建变量记录（带事务）
func (s *sVariable) BatchAddVariable(ctx context.Context, variables []*model.Variables) (int, error) {
	g.Log().Info(ctx, "批量新建变量记录", "count", len(variables))
	if len(variables) == 0 {
		return 0, nil
	}
	successCount := 0
	for _, variable := range variables {
		_, err := dao.Variables.Ctx(ctx).OmitEmpty().Data(variable).Insert()
		if err != nil {
			g.Log().Error(ctx, "批量新建变量记录失败", err, "varName", variable.VarName)
			continue
		}
		_, err = dao.Caching.Ctx(ctx).OmitEmpty().Data(variable).Insert()
		if err != nil {
			g.Log().Error(ctx, "批量同步缓存表失败", err, "varName", variable.VarName)
			continue
		}
		if variable.Scope != "sandbox" {
			_, err = dao.Dev.Ctx(ctx).Where(dao.Dev.Columns().Id, variable.DevID).Data(g.Map{
			...
			if err != nil {
				g.Log().Error(ctx, "批量更新设备标志失败", err)
			}
		}
		successCount++
	}
	g.Log().Info(ctx, "批量新建变量记录完成", "success", successCount, "total", len(variables))
