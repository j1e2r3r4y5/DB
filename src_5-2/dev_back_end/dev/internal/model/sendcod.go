...
type Func04Request struct {
	Scope    string  // 作用域: production/sandbox
	Entries  []Entry // 数据项列表
}
...
type OptimizationResult struct {
	OriginalPayload   int     // 原始payload（字节）
	OptimizedPayload  int     // 优化后payload（字节）
	SavedBytes        int     // 节省字节数
	SavedPercent      float64 // 节省百分比
	OriginalSegments  int     // 原始段数
	OptimizedSegments int     // 优化后段数
