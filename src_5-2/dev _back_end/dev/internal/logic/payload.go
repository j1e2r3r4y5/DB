...
func (s *sPayload) PayloadHandler(ctx context.Context, devSerial string, payload []byte) (isRepeat bool, Featurescode byte, DevUpdate *model.DevUpdateItem, LogUpdate *model.LogUpdateItem, err error) {
	...
	case "00":
		...
		WriteDevUpdateToInflux(ctx, org, bucket, DevUpdate, Featurescode)
		g.Log().Debug(ctx, fmt.Sprintf("心跳包处理完成: 设备[%s] 状态[在线]", devSerial))
		return false, Featurescode, DevUpdate, nil, nil
	...
	case "03":
		Featurescode = 0x03
		// 协议定义：2字节数据数量 + 循环解析数据项
		dataCountBytes, err := scanner.Next(2)
		if err != nil {
			g.Log().Error(ctx, "解析数据数量失败", err)
			return false, 0, nil, nil, err
		}
		dataCount := int(dataCountBytes[0])<<8 | int(dataCountBytes[1])
		g.Log().Debug(ctx, fmt.Sprintf("数据配置上发：设备[%s] 数据项数量[%d]", devSerial, dataCount))
	...
	case "05":
		Featurescode = 0x05
		// 协议定义：1字节功能码 + 2字节数据数量 N + N个数据项，每个项结构：
		// 1字节从站地址| 1字节数据类型| 2字节地址| 2字节数据长度（单位：字节）| 数据长度个字节数据
		dataCountBytes, err := scanner.Next(2)
		if err != nil {
			g.Log().Error(ctx, "解析数据数量失败", err)
			return false, 0, nil, nil, err
